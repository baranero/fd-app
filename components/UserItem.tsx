        ...
        h-max' key={data?.id}>
            <div className=" rounded-md   ">
                <div className="">
                        ...
                        { currentUser?.isAdmin === 'true' ? <button onClick={() => deleteButton(data.id, data.name)} className='bg-gray-500 w-full rounded-b-md py-3 hover:bg-gray-700 cursor-pointer transition'>Delete</button> : <></>}
                    </div>
            </div>
        </div>
    )
